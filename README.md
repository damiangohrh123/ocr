...
recc/
  cpp/                             # production C++ port
    ...
    automation/                    # Stage 2: goal-based decision and control, matches a rule
                                    # against real OCR output and generates the resulting KVM action sequence
...
`cpp/board_deploy/benchmark` is a pre-built, ready-to-run aarch64 binary — cross-compiled for the RK3588 board and statically linked against OpenCV 4.5.4 (core+imgproc only), Clipper/polyclipping, and zlib, so the board doesn't need any of those installed. The only runtime dependency it pulls in besides standard libc/libm/libpthread is `librknnrt.so`, which the board already provides at `/usr/lib`.

There is no cross-compile toolchain or aarch64-built OpenCV/Clipper/zlib set up on the board or dev machine — the pre-built binary above was produced by asking an AI assistant (Claude) to cross-compile it in its own sandboxed environment, since setting up a full aarch64 cross toolchain locally is a lot of one-time overhead. That environment (aarch64 gcc-9 toolchain matching the board's Ubuntu 20.04/glibc 2.31, plus statically-built OpenCV/Clipper/zlib) is cached in `cpp/cross_toolchain/aarch64-ubuntu20.04-toolchain.tar.gz` so a future rebuild skips the ~20+ minute bootstrap (steps to reuse it below). **If the C++ source changes, ask Claude to rebuild `cpp/board_deploy/benchmark` from the updated source** rather than trying to build locally, unless a proper local cross-compile environment gets set up (in which case, use the `cmake` invocation below and skip this whole workaround).

### Reusing the Cached Cross-Toolchain

`cpp/cross_toolchain/aarch64-ubuntu20.04-toolchain.tar.gz` contains an aarch64 cross-compiler matching the board's own toolchain era (gcc-9 / glibc 2.31 from Ubuntu 20.04 focal, plus host-side binutils) and statically-built dependencies for aarch64 (zlib, Clipper/polyclipping, and OpenCV 4.5.4 core+imgproc only).
...
```bash
pscp cpp/board_deploy/benchmark tpsadmin@192.168.1.101:/home/tpsadmin/board_deploy/benchmark
```
...
`ocr_server` (`cpp/api/`) loads the detection/recognition models once at startup, then serves OCR and alarm-detection results over HTTP instead of running once per invocation like `ocr`/`benchmark`.
...
```bash
pscp cpp/board_deploy/ocr_server tpsadmin@192.168.1.101:/home/tpsadmin/board_deploy/ocr_server
```
...
See `Internal_OCR_API_Proposal.docx` (repo root) for the original design rationale.

## Goal-Based Automation

`cpp/automation/` matches a person-written rule (a keyword to look for and what to do once found) against real OCR output, and generates the resulting KVM action sequence. It never touches an image, only the text and coordinates OCR already produced, so unlike everything else in `cpp/`, it has no OpenCV/RKNN dependency and isn't gated behind `BUILD_TOOLS` or a cross-compile toolchain — it builds and runs on any machine with a C++17 compiler:

```bash
cd cpp
g++ -std=c++17 -O2 -I. automation/*.cpp -o automation_runner
./automation_runner
```

It reads every rule from `rules/` and every real OCR result from `ocr_output/` (both plain JSON files, see `cpp/automation/rule.h` and `cpp/automation/screens.h` for the shapes), and matches each rule against each screen. Nothing is sent to a machine — the resulting sequence is printed and waits for a person to approve it.

## Python Reference Implementation

The original Python implementation (and its benchmark/test scripts and model-conversion tools) has been moved into its own standalone project, separate from this repo.
...
- Statically-linked OpenCV core+imgproc, Clipper, zlib (see `cpp/CMakeLists.txt`)

The Python reference implementation's requirements (Python 3.8+, `rknn-toolkit-lite2`, `opencv-python`, `numpy`, `shapely`, `pyclipper`) are documented in its own project now.
